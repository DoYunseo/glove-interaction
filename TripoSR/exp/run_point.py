...
for i, (image, image_id) in enumerate(images):
    ...
    if args.render:
        ...
        for ri, render_image in enumerate(render_images[0]):
            render_image.save(os.path.join(save_path, f"render_{ri:03d}.png"))
        save_video(render_images[0], os.path.join(save_path, f"render.mp4"), fps=30)
        timer.end("Rendering")
    ...
    out_mesh_path = os.path.join(save_path, f"mesh.{args.model_save_format}")
    if args.bake_texture:
        out_texture_path = os.path.join(save_path, "texture.png")
        ...
        xatlas.export(out_mesh_path, meshes[0].vertices[bake_output["vmapping"]],
                      bake_output["indices"], bake_output["uvs"],
        ...
        mesh_trimesh = trimesh.load(out_mesh_path, force='mesh')
        ...
        points, _ = trimesh.sample.sample_surface(mesh_trimesh, count=10000)
        ...
        # Save point cloud
        out_pcd_path = os.path.join(save_path, "point_cloud.ply")
        
        # Visualize point cloud
        o3d.visualization.draw_geometries([pcd])
        o3d.io.write_point_cloud(out_pcd_path, pcd)
        logging.info(f"Saved point cloud to {out_pcd_path}")
